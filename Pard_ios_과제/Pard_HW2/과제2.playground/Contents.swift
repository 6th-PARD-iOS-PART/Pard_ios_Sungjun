...
enum media {
    case Book(bookTitle: String)
    case Video(videoTitle: String)
    case Music(musicTitle: String)
}

let mybook: media = .Book(bookTitle: "정리하는 뇌")
let myvideo: media = .Video(videoTitle: "해리포터")
let mymusic: media = .Music(musicTitle: "주를 바라보며")
...
let align: week = .sunday
align.rawValue

let align2 = week(rawValue: 1)
...
func checkLoginStatus1(_ a: Bool, _ b: String) {
    if a == true {
        if b == "nil" {
            print("알 수 없는 사용자")
        }
...
checkLoginStatus1(true, "sungjun")

//MARK: - function guard문 사용 답안 작성란

func checkLoginStatus2(_ a: Bool, _ b: String) {
    guard a == true else {
        print("로그인이 필요합니다.")
        return
    }
...
let optValue: String = names.first ?? "no name found"
print(optValue)
